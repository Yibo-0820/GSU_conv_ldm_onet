...
class BetaVAE(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc_mu = nn.Linear(hidden_dims[-1] * 16,
                               latent_dim)  # for plane features resolution 128x128, spatial resolution is 4x4 after the last encoder layer
    ...
    def encode(self, enc_input: Tensor) -> List[Tensor]:
        ...
        result = torch.flatten(result, start_dim=1)  # ([B, D*16])
    ...
    def decode(self, z: Tensor) -> Tensor:
        ...
        result = self.decoder_input(z)  # ([B, D*16])
        result = result.view(-1, int(result.shape[-1] / 16), 4, 4)  # for plane features resolution 128x128, spatial resolution is 4x4 after the last encoder layer
        result = self.decoder(result)
        result = self.final_layer(result)  # ([B, D, resolution, resolution])
        return result
    ...
    def loss_function(self,
        ...
        return kld_weight * kl_loss.item()
